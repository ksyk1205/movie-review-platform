...
class MovieAcceptanceTest extends AcceptanceTest {
  ...
  void 영화_검색어_조회_테스트() {
    ...
    //when
    ExtractableResponse<Response> 영화_조회_결과 = MovieSteps.영화_검색어_조회_요청(title);

    //then
    assertThat(영화_조회_결과.statusCode()).isEqualTo(HttpStatus.OK.value());
    assertThat(영화_조회_결과.jsonPath().getList("title").get(0)).isEqualTo(title);
  }
